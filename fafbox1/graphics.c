...
void fillVRAM(uint8_t buffer, uint8_t color) {
	...
	CONTROL_PORT |= (1<<READ_ENABLE_PIN | 1<<WRITE_ENABLE_PIN | 1<<VIDEO_ENABLE_PIN | 1<<PERIPHERAL_ENABLE_PIN);

	//lower address is set as output (because it is used as input in peripheral controller cycle); higher address, data port, control port is always output
...
void drawPalette() {
	...
	for(uint8_t i = 0; i < 120; i++) {
		HIGHER_ADDRESS_PORT = i<<1;
		for(uint8_t j = 0; j < 240; j++) {
			color = (i/7)*16 + (j/15); //does not matter for now as resolution will change
			LOWER_ADDRESS_PORT = j;
			DATA_PORT = color;
